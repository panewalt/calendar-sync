...
class GCal:
    ...
    def __init__(self, ID, scope=None, appName=None, secretsFile=None, credentialsFile=None):
        ...
        self.credentials = self.get_credentials(scope, appName, secretsFile, credentialsFile)
        http = self.credentials.authorize(httplib2.Http())
        self.service = discovery.build('calendar', 'v3', http=http)
        # events_list = self.get_events_list()
        
        
    def get_credentials(self, scope, appName, secretsFile, credentialsFile):
...
def main():
    ...
    calendars = {
        ...
        'UL':  {'type': 'Outlook', 'appName':'Calendar Sync', 'user': 'pete@uledger.co', 'pwd': 'ULedger123', 'publishDetails': 'PA'}
        }
    ...
    for ID in calendars:
        ...
        if cal['type'] == 'Google':
            ...
            cal['instance'] = Outlook(ID=ID, user=cal['user'], pwd=cal['pwd'])
